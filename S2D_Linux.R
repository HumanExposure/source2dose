...
s2d = function(control.file="control_file.txt", number.of.houses= NULL) {
  ...
  eval.dermal.rates = function(fug.cvars,hp,prime,pucs,pucs.areas,prod.chem,nc) {
    ...
    hw    <- prime$hand.wash
    # ka and js require product layer thickness on skin
    ...
    source.id <- rep(pucs$source.id,nc)
    dtxsid    <- unlist(lapply(fug.cvars$dtxsid,rep,nrow(pucs)))
    ...
    for (c in 1:length(kaw)) {
      for (p in 1:nrow(pucs)) {
        rates$ka[p+(c-1)*nrow(pucs)] <- 1/(h[[p]]/phi.a[[c]] + h[[p]]/phi.w[[c]])
        rates$ks[p+(c-1)*nrow(pucs)] <- 1/(h[[p]]/kp[[c]]    + h[[p]]/phi.w[[c]])
      }
    }
    rates[is.na(rates)] <- 0
  ...
  eval.summary = function(direct,indirect,env.impact,run.name,house.num,nc,chem.list,chem.totals) {
    ...
    if(!is.null(nrow(env.impact))) { z <-select(env.impact,-daynum) } else {
  ...
  eval.use.data = function(pucs,prod.chem,fug.cvars,dermal.rates,puc.wipe.rinse) {
    ...
    for (prod in 1:nrow(pucs)) {
      ...
      ka     <- as.data.frame(dermal.rates)$ka/60                                       # Ka in [1/min] is volatilization rate from skin
  ...
  get.ran.vars = function(nc,np) {
    ...
    ran.prod <- y[-1]
    chem.rv        <- c("vapor","solub","kow","decay.air","decay.sur","diffus.air")
    z <- ""
    for (j in 1:nc) {
  ...
  S2D_Runner = function(i) {
    ...
    if (nrow(pucs)>0) {
      ...
      if (sum(use.chem)>0) {
        ...
        dermal.rates     <- eval.dermal.rates(fug.cvars,hp,prime,pucs,pucs.areas,prod.chem,nc)
        use.data         <- eval.use.data(pucs,prod.chem,fug.cvars,dermal.rates,puc.wipe.rinse)
        ...
        if (length(indoor.hrs)>0) {
          ...
          indirect       <- eval.indirect(d,chem.release,dermal.rates,hp,nc,prime,fug.cvars,chem.list)
        ...
        if (g$save.r.objects=="y" & any(env.impact[2:ncol(env.impact)]>0)) write.csv(env.impact,paste0(g$out,"/Temp/env.impact_",house.num,"_",run.name,".csv"))
      }  
      annual      <- eval.summary(direct,indirect,env.impact,g$run.name,house.num,nc,chem.list,chem.totals)
      if (g$save.r.objects=="y" & any(annual[3:ncol(annual)]>0)) write.csv(annual,paste0(g$out,"/Temp/annual_",house.num,"_",run.name,".csv"))
